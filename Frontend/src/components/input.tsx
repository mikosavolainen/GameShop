import '../App.css'
export default function Input({placeholder, className, label, width, icon, size, type}:{placeholder: string, className?: string, size: "small-white" | "normal-white" | "small-dark" | "normal-dark", type: "text" | "password", label?: string, icon: "search", width: number}){
    let string = ""
    switch (size){
        case "small-white":
    ...
    return(
        ...
        <div>
        {
            label && <label className='text-base block m-0 mb-1 text-wrench-neutral-white'>{label}</label>
        }
            ...
            <input type={`${type}`} className={`${string}`} placeholder={`${placeholder}`} size={width}/>
            </div>
        </div>
        </>
    )      
