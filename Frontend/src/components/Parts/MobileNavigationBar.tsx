...
export default function MobileNavigationBar({setMobileMenu, setMobileNotifications}: {setMobileMenu: React.Dispatch<React.SetStateAction<boolean>>, setMobileNotifications: React.Dispatch<React.SetStateAction<boolean>>}) {
  ...
  useEffect(() => setMobileMenu(false), [isMobileBreakpoint, setMobileMenu])
  ...
  return(
    <div className="md:hidden flex items-center justify-around w-full h-16 z-20 fixed left-0 right-0 bottom-0 bg-wrench-neutral-dark/85 backdrop-blur-3xl content-layout-padding">
      <Link href="/" className="flex flex-col items-center gap-0.5 w-[15%] group" onClick={() => {setMobileMenu(!true);setMobileNotifications(!true)}}> {/* don't replace !true with false, i will be really sad if you do - Nikolai */}
      ...
      <Link href="/search" className="flex flex-col items-center gap-0.5 w-[15%] group" onClick={() => {setMobileMenu(!true);setMobileNotifications(!true)}}>
      ...
      <button className="flex flex-col items-center gap-0.5 w-[15%] group" onClick={() => {setMobileMenu(!false);setMobileNotifications(!true)}}>
